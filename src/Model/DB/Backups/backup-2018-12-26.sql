...
DELIMITER ;;
CREATE DEFINER=`gabriel`@`localhost` PROCEDURE `create_book`(
IN pname varchar(100),
IN ptitle varchar(100),
IN pdescription text,
IN pid_publisher int,
IN pid_author int,
IN pcust_value float(6,2),
IN prent_value float(6,2),
IN psale_cust float(6,2),
IN pammount int,
IN pdate_create date,
IN pstatus_book int
)
BEGIN
	
	INSERT INTO books (name, title, description, id_publisher, id_author, cust_value, rent_value, ammount, date_create, status_book, sale_value) VALUES (pname, ptitle, pdescription, pid_publisher, pid_author, pcust_value, prent_value, pammount, pdate_create, pstatus_book, psale_cust);
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`gabriel`@`localhost` PROCEDURE `create_client`(
IN pname varchar(100),
IN pemail varchar(100),
IN ppassword varchar(128),
IN pphoto varchar(100),
IN pstatus_client int
)
BEGIN
	INSERT INTO clients (name, email, password, photo, status_client) VALUES (pname, pemail, ppassword, pphoto, pstatus_client);
	
    SET @id_client := (SELECT id_client FROM clients WHERE email = pemail);
    INSERT INTO adresses (id_client) VALUES (@id_client);
    INSERT INTO wallets (id_client) VALUES (@id_client);
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`gabriel`@`localhost` PROCEDURE `delete_book`(
IN pid_book int
)
BEGIN
	UPDATE books SET status_book = 0 
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`gabriel`@`localhost` PROCEDURE `delete_client`(
IN pid_client int
)
BEGIN
	UPDATE clients SET status_client = 0 WHERE id_client = pid_client;
    UPDATE wallets SET status_wallet= 0 WHERE id_client = pid_client;
    UPDATE adresses SET status_address= 0 WHERE id_client = pid_client;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`gabriel`@`localhost` PROCEDURE `update_book`(
IN pid_book int,
IN pname varchar(100),
IN ptitle varchar(100),
IN pdescription text,
...
IN pcust_value float(6,2),
IN prent_value float(6,2),
IN psale_value float(6,2),
IN pammount int,
IN pdate_create date,
IN pstatus_book int
)
BEGIN
	UPDATE books SET name = pname, title = ptitle, description = pdescription, id_publisher = pid_publisher, id_author = pid_author, cust_value = pcust_value, rent_value = prent_value, sale_value= psale_value, ammount = pammount, date_create = pdate_create, status_book = pstatus_book
    WHERE id_book = pid_book;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2018-12-26  0:55:03
